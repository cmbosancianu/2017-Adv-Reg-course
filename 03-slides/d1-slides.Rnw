...
df_qog <- read_xlsx(path = "../02-data/01-qog_std_cs_jan16.xlsx",
                    sheet = 2)
df_qog %<>%
    dplyr::select(cname, une_imr, wdi_gdppccon, ht_region) %>%
    mutate(cname = as.character(cname),
           wdi_gdppccon = wdi_gdppccon / 1000,
           africa = if_else(ht_region == 4, 1, 0))

write.table(dplyr::select(df_qog, cname, une_imr, wdi_gdppccon, africa),
            file = "../02-data/02-Infant-mort.txt",
            sep = "\t",
            row.names = FALSE,
            col.names = TRUE)
@

<<ch-2, eval=TRUE, echo=FALSE, warning=FALSE, message=FALSE, comment=NA, results='asis'>>=
df_qog <- read.table(file = "../02-data/02-Infant-mort.txt",
                     header = TRUE,
                     sep = "\t")
df_qog %<>%
    mutate(wdi_gdppccon = wdi_gdppccon - 5)
model1 <- lm(une_imr ~ wdi_gdppccon + africa,
             na.action = na.omit,
             data = df_qog)
...
texreg(list(model1),
       ...
       label = "tab:tab-02", fontsize = "footnotesize",
       custom.note = "%stars. GDP/capita rescaled by subtracting 5,000 USD.")
@
...
<<ch-4, eval=TRUE, echo=FALSE, warning=FALSE, message=FALSE, comment=NA, results='asis'>>=
df_qog %<>%
    mutate(wdi_gdppccon = (wdi_gdppccon + 5) * 1000,
           lngdp = log(wdi_gdppccon))

model2 <- lm(une_imr ~ lngdp + africa,
             na.action = na.omit,
             data = df_qog)
...
texreg(list(model1,model2),
       ...
       fontsize = "scriptsize",
       custom.note = "%stars. GDP/capita used in its original form.")
@
...
<<ch-6, echo=FALSE, eval=TRUE, warning=FALSE, comment=NA, message=FALSE>>=
...
<<ch-7, echo=FALSE, eval=FALSE, warning=FALSE, comment=NA, message=FALSE>>=
pdf(file = "../04-graphs/01-04.pdf",
    height = 5, width = 7)
...
pdf(file = "../04-graphs/01-05.pdf",
    height = 5, width = 7)
plot(df_cal$college, studres(model1),
     xlab = "% college (centered)",
     ylab = "Studentized residuals")
abline(h = 0, lty = 2)
lines(lowess(df_cal$college, studres(model1)),
      col = "blue")
dev.off()
@
...
<<ch-8, echo=FALSE, eval=TRUE, warning=FALSE, comment=NA, message=FALSE, results='asis'>>=
...
<<ch-9, echo=FALSE, eval=FALSE, warning=FALSE, comment=NA, message=FALSE>>=
pdf(file = "../04-graphs/01-06.pdf",
    height = 5, width = 7)
plot(fitted(model1), studres(model1),
     xlab = "Fitted values",
     ylab = "Studentized residuals")
...
pdf(file = "../04-graphs/01-07.pdf",
    height = 5, width = 7)
plot(df_boston$rm, studres(model1),
     xlab = "Ave. room no.",
     ylab = "Studentized residuals")
abline(h = 0, lty = 2)
lines(lowess(df_boston$rm, studres(model1)),
      col = "blue")
dev.off()
@
...
<<ch-11, echo=FALSE, eval=FALSE, warning=FALSE, comment=NA, message=FALSE>>=
df_bill92 <- read.table(file = "../02-data/05-billionaires1992.txt",
                        header = TRUE, sep = "\t",
                        na.strings = "NA")
df_bill92 %<>%
    mutate(region = as.character(region),
           region = case_when(region == "M" ~ "Middle East",
                              region == "U" ~ "United States",
                              region == "A" ~ "Asia",
                              region == "E" ~ "Europe",
                              region == "O" ~ "Other"))
...
model2 <- lm(democrat ~ coll_cent,
             data = df_cal,
             na.action = na.omit)

pdf("../04-graphs/01-09.pdf",
    height = 5, width = 7)
qqPlot(model2,
       xlab = "t distribution",
       ylab = "Studentized residuals",
...
model3 <- lm(1/wealth ~ age,
             data = df_bill92)
pdf("../04-graphs/01-10.pdf",
    height = 5, width = 7)
qqPlot(model3,
       xlab = "t distribution",
       ylab = "Studentized residuals",
